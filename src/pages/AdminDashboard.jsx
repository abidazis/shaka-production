...
export default function AdminDashboard({ onLogout }) {
  ...
  // Referensi khusus untuk mereset input file gambar
  ...
  // FUNGSI UPLOAD GAMBAR KE SANITY
  ...
  // FUNGSI SIMPAN DATA (DILENGKAPI ERROR CATCHER)
  const handleSaveData = async (e) => {
    ...
    try {
      ...
      // 2. Siapkan kerangka data (Document)
      let doc = {};
      ...
      // 3. Eksekusi tembak ke database Sanity
      await client.create(doc);
      
      // 4. Jika sukses, muncul pop up dan reset form
      alert("✅ MANTAP! Data berhasil disimpan ke database Sanity.");
      setInputName(""); 
      setInputPrice(""); 
      setInputDesc(""); 
      setInputImage(null);
      if (fileInputRef.current) fileInputRef.current.value = ""; // Reset tulisan file yg dipilih
      
      fetchData(); // Refresh data live

    } catch (err) {
      // INI PENTING: Menangkap error dan menampilkannya langsung ke kamu
      console.error("DETAIL ERROR DARI SANITY:", err);
      ...
      <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col justify-between p-6 fixed h-full z-50">
        <div className="flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            <button onClick={() => setActiveMenu("product")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all ${activeMenu === 'product' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><Package size={16} /> Kelola Produk</button>
            <button onClick={() => setActiveMenu("portfolio")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all ${activeMenu === 'portfolio' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><ImageIcon size={16} /> Kelola Portfolio</button>
            <button onClick={() => setActiveMenu("testimonial")} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all ${activeMenu === 'testimonial' ? 'bg-red-600 text-white shadow-lg shadow-red-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}><MessageSquare size={16} /> Kelola Testimoni</button>
              ...
              <form onSubmit={handleSaveData} className="flex flex-col gap-4">
                ...
                {activeMenu !== "testimonial" && (
                  <div>
                    ...
                    <input required type="file" accept="image/*" onChange={(e) => setInputImage(e.target.files[0])} ref={fileInputRef} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-[10px] text-slate-500 file:bg-slate-900 file:border-0 file:text-white file:rounded-md file:px-2 file:py-1 file:mr-2 cursor-pointer" />
                  </div>
                )}
                ...
                <div className="max-h-[400px] overflow-y-auto pr-2 flex flex-col gap-2 custom-scrollbar">
                  {activeMenu === "product" && products.map(item => (
                    <div key={item._id} className="flex justify-between items-center bg-slate-950 p-3.5 rounded-xl border border-slate-900">
                      <div className="flex items-center gap-4"><img src={item.image ? urlFor(item.image).url() : "/placeholder.jpg"} className="w-10 h-10 object-cover rounded-lg" /><div><h4 className="font-bold text-xs text-slate-200">{item.name}</h4><p className="text-[10px] text-red-400 font-bold mt-0.5">{item.price}</p></div></div>
